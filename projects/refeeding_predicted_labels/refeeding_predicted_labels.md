...
<img src="https://render.githubusercontent.com/render/math?math=r_{i+1}-r_i = \frac{\sum_{j=1}^{i+1} r_{j-1}(n_j-n_{j-1})}{n_{i+1}} - \frac{\sum_{j=1}^i r_{j-1}(n_j-n_{j-1})}{n_i} \geq \frac{\sum_{j=1}^{i+1} r_{j-1}(n_j-n_{j-1})}{n_{i}} - \frac{\sum_{j=1}^i r_{j-1}(n_j-n_{j-1})}{n_i} = r_{i}(n_{i+1}-n_i) \geq 0">
